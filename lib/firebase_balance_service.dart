...
class FirebaseBalanceService {
  ...
  static Future<Map<String, dynamic>> deductFromBalance({
    ...
    try {
      ...
      // Create transaction record with regular Timestamp (not FieldValue.serverTimestamp)
  ...
  static Future<List<Map<String, dynamic>>> getTransactionHistory() async {
    try {
      ...
      if (balanceDoc.exists) {
        final data = balanceDoc.data()!;
        final transactions = data['transactionHistory'] as List<dynamic>? ?? [];
  ...
  // Manual balance reset (for testing)
  static Future<void> resetBalance() async {
    try {
      ...
      print('Balance manually reset to ₹$MONTHLY_BALANCE');
    } catch (e) {
